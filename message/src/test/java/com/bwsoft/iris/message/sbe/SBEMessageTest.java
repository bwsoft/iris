...
public class SBEMessageTest {
	...
	/**
	 * Verify the SBEMessage parser by parsing the sbeBuffer populated
	 * by RL SBE encoder. 
	 * 
	 * @throws FileNotFoundException
	 * @throws JAXBException
	...
	public void sbeMessageTest() throws FileNotFoundException, JAXBException {
		...
		int testCase = 0;
		while( null != obj ) {
			System.out.println("start testing case #"+testCase);
			
			// it is the message belong to this message schema.
			// obtain the message defintion 
			SBEMessage msg = (SBEMessage) obj.getDefinition();
			int nsize = obj.getSize(); // size of the message. message header is not included. 
			
			// print out message ID
			System.out.println("Get a message with template ID: "+msg.getID()+", with size: "+nsize);
			Assert.assertEquals((short)1, msg.getID());

			System.out.println(obj.toString()); // display whole object in json
			Assert.assertEquals(sbeMsgInJson[testCase], obj.toString());
			...
			if( testCase == 0 ) {
				...
				Assert.assertEquals(true, obj.isSet(extraField, "sunRoof"));
				Assert.assertEquals(false, obj.isSet(extraField, "sportsPack"));
				Assert.assertEquals(true, obj.isSet(extraField, "cruiseControl"));				
			}
